...
export default function SiteHeader() {
	...
	useGSAP(() => {
		ScrollTrigger.create({
			...
			onLeaveBack: () => headerRef.current?.classList.remove('border-b')
		})
	})
	...
	return (
		<header
			className={cn(
				'top-0 z-40 border-border/60 bg-background/80 backdrop-blur transition-[border]',
				location.pathname === '/' ? '' : 'sticky'
			)}
			ref={headerRef}
		>
			<AnimatedContent reverse={true} duration={1} distance={100}>
